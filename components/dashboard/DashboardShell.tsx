...
/**
 * Profile data pre-fetched server-side to eliminate client waterfall
 */
...
function AppSidebar({ profile }: { profile: UserProfile | null }) {
                        ...
                        <DropdownMenuTrigger asChild>
                            ...
                            >
                                ...
                                <div className="flex flex-col items-start text-sm truncate">
                                    <span className="font-semibold text-white text-sm leading-tight">{profile?.full_name || profile?.email}</span>
...
function Topbar({ profile }: { profile: UserProfile | null }) {
    ...
    const orgName = profile?.organizations?.name
        || (profile?.email ? profile.email.split('@')[1]?.split('.')[0] : null)
        || 'My Workspace'
 ...
 * Receives pre-fetched profile data from the Server Component layout,
 * eliminating the client-side auth + profile fetch waterfall.
 */
