...
export default function MonitoringClient({ fallbackData }: { fallbackData: any }) {
    const { profile } = useUser()
    const supabase = createClient()
    const [data, setData] = useState<MonitoringData>(fallbackData || EMPTY_MONITORING)
                    ...
                    <CardContent className="pt-2">
                        <ResponsiveContainer width="100%" height={180}>
                            <AreaChart data={data?.uptimeTrend || []} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                                ...
                                <Tooltip
                                    formatter={(v: any) => [`${v.toFixed(1)}%`, 'Uptime']}
