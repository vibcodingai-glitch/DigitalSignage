...
export const WelcomeBanner = React.memo(function WelcomeBanner({
    firstName, onlineCount, totalCount, onlinePct
}: {
    firstName: string
    onlineCount: number
    totalCount: number
            ...
            <div className="relative flex items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-blue-200 text-sm mt-1">
                        Your network has <span className="font-semibold text-white">{onlineCount} screen{onlineCount !== 1 ? 's' : ''}</span> broadcasting live right now.
                    </p>
                </div>
                <div className="hidden sm:flex flex-col items-end gap-1 shrink-0">
                    ...
                    <span className="text-blue-200 text-xs">{onlineCount} / {totalCount} online</span>
                </div>
            </div>
        </div>
    )
...
export const UptimeBar = React.memo(function UptimeBar({
    onlinePct, onlineCount, totalCount, offlineCount, unassignedCount, onRefresh
}: {
    onlinePct: number
    onlineCount: number
    totalCount: number
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4 rounded-xl bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 shadow-sm">
            <div className="flex items-center gap-3 shrink-0">
                ...
                <div>
                    ...
                    <p className="text-[11px] text-slate-400">{onlineCount} of {totalCount} screens online</p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-4 text-xs shrink-0">
                <span className="flex items-center gap-1.5 text-emerald-600 dark:text-emerald-400 font-semibold">
                    <CheckCircle2 className="h-3.5 w-3.5" /> {onlineCount} Online
                </span>
                <span className="flex items-center gap-1.5 text-red-500 font-semibold">
                    <XCircle className="h-3.5 w-3.5" /> {offlineCount} Offline
                </span>
                <span className="flex items-center gap-1.5 text-amber-500 font-semibold">
                    <AlertCircle className="h-3.5 w-3.5" /> {unassignedCount} Unassigned
                </span>
                <Button variant="ghost" size="sm" className="h-7 text-xs gap-1 text-slate-500" onClick={onRefresh}>
...
export const ScreenStatusTable = React.memo(function ScreenStatusTable({
    screens, isLoading
}: {
    screens: RecentScreen[] | null
    isLoading: boolean
}) {
    return (
...
export const LivePreviewPanel = React.memo(function LivePreviewPanel({
    screens, isLoading
}: {
    screens: RecentScreen[] | null
    isLoading: boolean
